...
if __name__ == '__main__':
    ...
    images, poses, render_poses, hwf, i_split, K, bds_dict = utils.load_datasets(
        args)
    bds_dict = {
        'near': 0.05,
        'far': 0.15
    ...
    grasp_points = torch.tensor([[0.09, 0.09, -0.025], [-0.09, 0.09, 0.025],
    ...
    # Setup CEM cost.
    cem_cost = lambda x: grasp_opt.grasp_cost(x, n_f, coarse_model, fine_model,
                                              renderer)

    # Optimize points with CEM.
    with torch.no_grad():
        mu_f, Sigma_f = grasp_opt.optimize_cem(cem_cost,
                                               mu_0,
                                               Sigma_0,
                                               num_iters=25,
                                               elite_frac=.1,
