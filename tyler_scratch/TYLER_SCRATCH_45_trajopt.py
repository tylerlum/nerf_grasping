...
from pydrake.solvers import BoundingBoxConstraint, SnoptSolver, Solve, SolverId, SolverOptions, SolverType
...
def load_srdf_disabled_collisions(srdf_file, plant, collision_filter_manager):
    ...
    for disable_collisions in robot.iter('disable_collisions'):
        allowed_collision_pair = [
            disable_collisions.get('link1'), disable_collisions.get('link2')
        ]
...
# %%
ALLEGRO_ROS2_PATH = Path(nerf_grasping.get_package_root()) / "fr3_algr_ik" / "allegro_ros2"
class AllegroFR3TrajOpt:
    ...
    def setup_vanilla_allegro_fr3(self):
        ...
        self.plant.WeldFrames(
            self.plant.world_frame(), obj_body.body_frame(), X_WO
        )
        ...
        for geom_name in ["algr_rh_if_ds", "algr_rh_mf_ds", "algr_rh_rf_ds", "algr_rh_th_ds"]:
    ...
    def impose_self_collision_constraints(self):
        ...
        for s in evaluate_at_s:
            if s >= self.cfg.s_start_self_col:
                self.trajopt.AddPathPositionConstraint(self.self_collision_constraint, s)
    ...
    def impose_finger_obj_collision_constraints(self):
        ...
        evaluate_at_s = np.linspace(0, 1, self.cfg.num_control_points)
        for s in evaluate_at_s:
            finger_names = ["algr_rh_if_ds", "algr_rh_mf_ds", "algr_rh_rf_ds", "algr_rh_th_ds"]
...
def solve_traj_opt(
    ...
    T_traj = opt_result.GetSolution(trajopt.trajopt.duration())  # rescale the traj length
...
q_robot_0 = np.array([ 0.05298999,  0.40301091,  0.10604012, -2.09894103, -0.91767794,
        2.12433254,  1.33983849,  0.06403663,  0.80356938,  0.24809591,
        0.06735399,  0.27015063,  0.74815333,  0.20341073,  0.3725833 ,
        0.3380464 ,  0.68399602,  0.5206812 ,  0.4830313 ,  1.26247382,
        0.11839718, -0.01316089,  0.05241748])
