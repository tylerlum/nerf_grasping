...
def get_camera_orientation(camera_location, target_view_point=np.array([0,0,0])):
    ...
    """
    view_dir = ((target_view_point - camera_location)
                / la.norm(target_view_point - camera_location, 2))
    z_bar = np.reshape(-view_dir, (3,))
    x_bar = np.reshape([view_dir[1], -view_dir[0], 0], (3, ))

    if la.norm(x_bar, 2) < 10**(-8):
        x_bar = np.array([1., 0., 0.])
    else:
...
def generate_random_R():
    ...
    """
    x =  np.random.rand(3)
    v = np.reshape([np.cos(2*math.pi*x[1])*np.sqrt(x[2]), \
...
def gen_data_on_sphere(num_pnts, cam_dist):
    ...
    for i, cam_loc in enumerate(camera_locations_sphere):
        camera_locations_sphere[i] = (np.asarray(camera_locations_sphere[i])
                                      * cam_dist, "origin", 0)
...
def nerf_object_training_data_generator(object_name, save_dir, prms):
    ...
    for mode, num_imgs in zip(['train', 'test', 'val'], [
        ...
        json_data['obj_max_dim'] = la.norm(
            np.array(obj.dimensions)) # longest dist through object
        ...
        for f in existing_files:
            os.remove(f)
        output_dir_json = os.path.abspath(save_dir + "/nerf_training_data/transforms_"
                                          + mode + ".json")
        ...
        if mode == "train":
            for i in [-1, 1]:
                for j in [-1, 1]:
                    for k in [-1, 1]:
                        ...
                        camera_locations.append( (xyz_dir * prms["cam_offset_far"],
                                                  "origin", 0) )
            ...
            for i, (dim0, dim1) in enumerate(dim_arr):
                ...
                camera_locations.extend( [(xyz, "origin", 0) for xyz in np.insert(
                    np.zeros((2,2)), i, dim2*np.array([1,-1]), axis=0).T] )

            camera_locations.extend(gen_data_on_sphere(num_pnts=(
                num_imgs - len(camera_locations)), cam_dist=prms["cam_offset_far"]))

        else: #  (or non-training run for rect)
            # calculate even spread of locations around unit sphere
            camera_locations = gen_data_on_sphere(num_pnts=num_imgs,
                                                  cam_dist=prms["cam_offset_far"])

        # Next, actually render the images at each location
        org = np.array([0,0,0])
        for step, (camera_loc, orient_mode, axis_to_zero) in enumerate(camera_locations):
            frame = {}
            ...
            frame['file_path'] = os.path.join("./", mode, output_file_pattern_string[:-4].format(step))
...
if __name__ == '__main__':
    ...
    print("Starting nerf_object_training_data_generator [running python {}]".format(sys.version_info[0]))
    ...
    prms = {"teddy_bear"  : {"cam_offset_near" : 0.55, "cam_offset_far" : 0.75, "num_train" : 14+100, "num_test": 10, "num_val" : 2},
            "dining_table": {"cam_offset_near" : 0.65, "cam_offset_far" : 3.65, "num_train" : 14+175, "num_test": 10, "num_val" : 2},
            "laptop"      : {"cam_offset_near" : 0.50, "cam_offset_far" : 0.75, "num_train" : 14+100, "num_test": 10, "num_val" : 2},
            "mug"         : {"cam_offset_near" : 0.30, "cam_offset_far" : 0.30, "num_train" : 14+100, "num_test": 10, "num_val" : 2}}
    ...
    if object_name is None or np.any(np.abs(bpy.data.objects[object_name].dimensions) < 10**-7):
        print("No recognizable objects in the scene, make sure info is added to the dictionary!\n")
    else:
        print("Generating data for object: {}".format(object_name))
        data_dir = "/home/adam/Documents/song/single_object_blenders/"
        nerf_object_training_data_generator(prms=prms[object_name],
                                            object_name = object_name,
                                            save_dir=os.path.join(os.path.dirname(data_dir), object_name))
    print("--------------------    DONE NERF TRAINING DATA SCRIPT    --------------------")